...
class AgentData:
    ...
    def __init__(
        ...
        self.top_k_logprobs: list[list[float]] = []
        self.top_k_token_ids: list[list[int]] = []
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        output = AgentLoopOutput(
            ...
            top_k_logprobs=agent_data.top_k_logprobs[: self.response_length] if agent_data.top_k_logprobs else None,
            top_k_token_ids=agent_data.top_k_token_ids[: self.response_length] if agent_data.top_k_token_ids else None,
    ...
    async def _handle_generating_state(
        ...
        if output.top_k_logprobs:
            agent_data.top_k_logprobs += output.top_k_logprobs
        elif agent_data.top_k_logprobs:
            agent_data.top_k_logprobs += [[] for _ in range(len(agent_data.response_ids))]
        if output.top_k_token_ids:
            agent_data.top_k_token_ids += output.top_k_token_ids
        elif agent_data.top_k_token_ids:
            agent_data.top_k_token_ids += [[] for _ in range(len(agent_data.response_ids))]
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if agent_data.top_k_logprobs:
            agent_data.top_k_logprobs += [[] for _ in range(len(response_ids))]
        if agent_data.top_k_token_ids:
            agent_data.top_k_token_ids += [[] for _ in range(len(response_ids))]
