...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        if self.config.reward_logprobs > 20:
            # vLLM defaults to max_logprobs=20, but CDE may require larger top-k.
            engine_kwargs.setdefault("max_logprobs", int(self.config.reward_logprobs))
    ...
    async def generate(
        ...
        calculate_log_probs = bool(sampling_params.pop("logprobs", False))
        reward_logprobs = int(self.config.get("reward_logprobs", 0))
        if reward_logprobs > 0:
            sampling_params["logprobs"] = reward_logprobs
        else:
            sampling_params["logprobs"] = 0 if calculate_log_probs else None
        ...
        if sampling_params.logprobs is not None:
            log_probs = []
            if final_res.outputs[0].logprobs is not None:
                for i, logprobs_dict in enumerate(final_res.outputs[0].logprobs):
                    token_id = token_ids[i] if i < len(token_ids) else None
                    if token_id is not None and logprobs_dict is not None and token_id in logprobs_dict:
                        log_probs.append(float(logprobs_dict[token_id].logprob))
                    else:
                        log_probs.append(0.0)
            if len(log_probs) == 0:
                log_probs = None

        if reward_logprobs > 0 and final_res.outputs[0].logprobs is not None:
            top_k_logprobs = []
            top_k_token_ids = []
            for logprobs_dict in final_res.outputs[0].logprobs:
                if not logprobs_dict:
                    top_k_logprobs.append([])
                    top_k_token_ids.append([])
                    continue

                sorted_items = sorted(logprobs_dict.items(), key=lambda x: x[1].logprob, reverse=True)
                top_k_token_ids.append([int(tok_id) for tok_id, _ in sorted_items])
                top_k_logprobs.append([float(logprob_obj.logprob) for _, logprob_obj in sorted_items])
