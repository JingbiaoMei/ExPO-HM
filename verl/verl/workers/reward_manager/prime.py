...
class PrimeRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        raw_extra_info = data.non_tensor_batch.get("extra_info", None)
        if raw_extra_info is None:
            extra_info = [None] * len(data_sources)
        elif isinstance(raw_extra_info, dict):
            extra_info = [raw_extra_info] * len(data_sources)
        else:
            try:
                extra_info = list(raw_extra_info)
            except TypeError:
                extra_info = [raw_extra_info] * len(data_sources)

        top_k_logprobs = data.non_tensor_batch.get("top_k_logprobs", None)
        top_k_token_ids = data.non_tensor_batch.get("top_k_token_ids", None)
        if top_k_logprobs is not None and top_k_token_ids is not None:
            enriched_extra_info = []
            for i in range(len(data_sources)):
                item_info = extra_info[i] if i < len(extra_info) else None
                item_info = dict(item_info) if item_info is not None else {}
                item_info.update(
                    {
                        "top_k_logprobs": top_k_logprobs[i] if i < len(top_k_logprobs) else None,
                        "top_k_token_ids": top_k_token_ids[i] if i < len(top_k_token_ids) else None,
                        "tokenizer": self.tokenizer,
                    }
                )
                enriched_extra_info.append(item_info)
            extra_info = enriched_extra_info
