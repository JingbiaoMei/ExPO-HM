...
def _collect_reward_component_metrics(batch: DataProto) -> dict[str, float]:
    """Collect mean/max/min metrics for numeric reward extra fields."""
    ...
    cde_keys = {
        ...
        "no_logit",
        "logit_difference",
        "full_vocab_num_tokens",
        "full_vocab_max_prob",
        "full_vocab_min_prob",
        "full_vocab_effective_size",
    }
    skip_keys = {
        "uid",
        "raw_prompt",
    ...
    for key, values in batch.non_tensor_batch.items():
        ...
        if isinstance(values, np.ndarray):
            ...
            if values.dtype == object:
                try:
                    ...
                    continue
            elif np.issubdtype(values.dtype, np.number) or values.dtype == np.bool_:
        ...
        prefix = "critic/cde" if key.startswith("cde_") or key in cde_keys else "critic/reward_components"
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    metrics.update(_collect_reward_component_metrics(batch))
